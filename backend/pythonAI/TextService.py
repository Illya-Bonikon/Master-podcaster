...
def generate_text(prompt: str, model: str = "gpt-4o-mini", max_tokens: int = 4096, temperature: float = 1, top_p: float = 1) -> str:
    messages = [
        {
            "role": "system",
            "content": "Використовуй лише українські букви та знаки, заміни всі числа та англійські букви українським текстом. Напиши епізод подкасту на 500 слів на наступну тему:"
        },
        {
            "role": "user",
            "content": prompt,
...
def summarize_text(text: str) -> str:
    prompt = (
        "Напиши максимально короткий (до 50 слів) опис українською, про що йдеться в наступному епізоді багатоепізодового подкасту:\n\n"
        f"{text}"
    )
